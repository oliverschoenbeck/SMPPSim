...
public class TestProtocolHandler1 extends StandardProtocolHandler {
	...
	void getSubmitSMResponse(byte[] message, int len) throws Exception {
		...
		// Validate MSISDN
		try {
			logger.info("Validating destination_addr:" + smppmsg.getDestination_addr());
			...
			logger.warning("'Invalid' MSISDN. This protocol handler treats non-numeric MSISDN as invalid for testing purposes");
			smsc.incSubmitSmERR();
			resp_message =
				smppresp.errorResponse(
					smppresp.getCmd_id(),
